...
def is_torch_available() -> bool:
    try:
        ...
        if not is_available:
            return False
        # Accept NVIDIA container builds (nv24.xx) which are based on PyTorch 2.3
        # but have 2.4+ features backported
        if "nv24" in torch_version or "nv25" in torch_version:
            return True
        parsed_version = version.parse(torch_version)
        if parsed_version < version.parse("2.4.0"):
            logger.warning_once(f"Disabling PyTorch because PyTorch >= 2.4 is required but found {torch_version}")
        return version.parse(torch_version) >= version.parse("2.4.0")
