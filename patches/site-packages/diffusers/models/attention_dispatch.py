...
def _native_attention(
    ...
    if _parallel_config is None:
        ...
        # enable_gqa was added in PyTorch 2.5; handle gracefully on 2.3/2.4
        if enable_gqa and query.shape[1] != key.shape[1]:
            # GQA: manually expand k/v heads to match q heads
            q_heads = query.shape[1]
            kv_heads = key.shape[1]
            if q_heads % kv_heads == 0:
                ratio = q_heads // kv_heads
                _sdpa_kwargs["key"] = _sdpa_kwargs["key"].repeat_interleave(ratio, dim=1)
                _sdpa_kwargs["value"] = _sdpa_kwargs["value"].repeat_interleave(ratio, dim=1)
        try:
            out = torch.nn.functional.scaled_dot_product_attention(**_sdpa_kwargs, enable_gqa=enable_gqa)
        except TypeError:
            out = torch.nn.functional.scaled_dot_product_attention(**_sdpa_kwargs)
        out = out.permute(0, 2, 1, 3)
    else:
        out = _templated_context_parallel_attention(
            query,
